...
@interface RNFrostedSidebar () <UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate>
{
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        tableViewRect = CGRectMake(0, 0, 0, 400);
        _contentView = [[UITableView alloc] initWithFrame:tableViewRect style:UITableViewStyleGrouped];
...
//        [_contentView addGestureRecognizer:_tapGesture];
...
- (instancetype)initWithTopController:(UIViewController*)controller {
//    NSAssert(NO, @"Unable to create with plain init.");
    return [self initWithImages:nil selectedIndices:nil borderColors:nil];
//    self setn = controller.navigationController;
}
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    _searchBar.frame = CGRectMake(0, 34, 0, 40);
    ...
    void (^animations)() = ^{
//        self.contentView.frame = contentFrame;
        self.contentView.frame = CGRectMake(0, 74, contentFrame.size.width, contentFrame.size.height - 74);
        [_contentView setContentOffset:CGPointMake(0, 0) animated:NO];
        _searchBar.frame = CGRectMake(0, 34, 200, 40);;
...
{
    if ([touch.view isDescendantOfView:_contentView] || [touch.view isDescendantOfView:_searchBar]) {
...
//    id user = [tableViewData objectAtIndex:[indexPath row]];
    cell.textLabel.text = @"Name User";
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self didTapItemAtIndex:[indexPath section]];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    NSArray *visibleCells = [_contentView visibleCells];
    ...
    if (visibleCells != nil  &&  [visibleCells count] != 0) {       // Don't do anything for empty table view
        ...
        NSInteger tableViewTopPosition = _contentView.frame.origin.y;
        NSInteger tableViewBottomPosition = _contentView.frame.origin.y + _contentView.frame.size.height;
        
        /* Get content offset to set opacity */
        CGRect topCellPositionInTableView = [_contentView rectForRowAtIndexPath:[_contentView indexPathForCell:topCell]];
        CGRect bottomCellPositionInTableView = [_contentView rectForRowAtIndexPath:[_contentView indexPathForCell:bottomCell]];
        CGFloat topCellPosition = [_contentView convertRect:topCellPositionInTableView toView:[_contentView superview]].origin.y;
        CGFloat bottomCellPosition = ([_contentView convertRect:bottomCellPositionInTableView toView:[_contentView superview]].origin.y + cellHeight);
...
#pragma mark - my add
- (void) addSearchBar:(id)anyObj
{
    _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 34, 200, 40)];
    _searchBar.searchBarStyle = UISearchBarStyleMinimal;
    _searchBar.translucent = YES;
    _searchBar.placeholder = @"search user";
    for (UIView *subView in self.searchBar.subviews)
    {
        for (UIView *secondLevelSubview in subView.subviews){
            if ([secondLevelSubview isKindOfClass:[UITextField class]])
            {
                ...
                searchBarTextField.textColor = [UIColor lightTextColor];
                break;
            }
        }
    }
    _searchBar.delegate = anyObj;
    
    [self.view addSubview:_searchBar];
    
    self.contentView.frame = CGRectMake(0, 0, 400, 100);
    
    //    self.contentView.frame = CGRectOffset( self.contentView.frame, 100, 10 );
